...
class GarminConnectConfigManager(JsonConfig):
    ...
    def get_secure_password(self):
        ...
        if system == "Darwin":
            ...
            try:
                password_command = self.__get_node_value(
                    "credentials", "password_command"
                ).split()
                ...
                pass
            raise ConfigException(
                "password_command missing in json file or command not valid"
            )
    ...
    def enabled_stats(self):
        ...
        if not self.enabled_statistics:
            json_enabled_stats_dict = self.config.get(
                "enabled_stats", {stat_name: True for stat_name in list(Statistics)}
            )
